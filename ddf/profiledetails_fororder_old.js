        ...
        return {
            ...
            return itemPrice;
              ...
              downloadPDF:function(data){
                ...
                pdfData.telephoneNumber = getDynamicProperty('_phoneCode',self.user().dynamicProperties) + getDynamicProperty('_phoneNumber',self.user().dynamicProperties);
                pdfData.emailId=self.user().email();
                pdfData.nationality = getDynamicProperty('_nationality',self.user().dynamicProperties);
                ...
                if (getDynamicProperty('_passportNumber',self.user().dynamicProperties)) {
                    pdfData.passportNumber = getDynamicProperty('_passportNumber',self.user().dynamicProperties);
                        pdfData.labelForPassportOrEmiratesId="PassportNo"
                }
                if (getDynamicProperty('_emiratesId',self.user().dynamicProperties)) {
                    pdfData.passportNumber = getDynamicProperty('_emiratesId',self.user().dynamicProperties);
                ...
                var bikeCounter = 0;
                 $.ajax({
                        ...
                        error:function(errorRes){
                            if(errorRes && errorRes.responseJSON){
                        ...
                        if(purcahsedTickets){
                            var newData = JSON.parse(JSON.stringify(purcahsedTickets));
                            newData.forEach(function(ticket, index) {
                            ticket['largeImageURLs'] = ["/file/general/no-image.jpg"];
                            });
                            purcahsedTickets = newData;
                            productCompareSharedViewModel.currentProfileRaffleTickets(purcahsedTickets);
                            widget.destroySpinner(ticketCounts);
                                ...
                                try {
                                    ...
                                    if(productIds){
                                      widget._modifyTicketDetails(productIds,purcahsedTickets);
                                        ...
                                        widget.destroySpinner(ddfTicketsSummary, ddfTicketsSummarySection);
                                        widget.destroySpinner(ticketCounts, ticketCountsSection);
                                    }
                                     /*DDF-466 ends here*/
                                }
                                catch(err) {
