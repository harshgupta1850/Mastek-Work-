...
define(
  ...
  function(ko, ccRestClient, CCConstants, pubsub, CCi18n, log, SkuPropertiesHandler, CCStoreConfiguration) {
    ...
    function ProductViewModel(pProduct, params) {
      ...
      if( params && params.hasOwnProperty('externalPrice')){
        self.externalPrice = params.externalPrice;
      }
      if(params && params.hasOwnProperty('quantity')){
    ...
    ProductViewModel.prototype.getProductsAvailability = function(success, error, productIds, catalogId, locationIds) {
      var input = {};
      input[CCConstants.PRODUCTS_PARAM] = productIds;
      input[CCConstants.CATALOG] = (catalogId == null ? '' : catalogId);
      if(locationIds && null !== locationIds && '' !== locationIds) {
        input[CCConstants.LOCATION_IDS] = locationIds;
      }
      ...
      if (product.addOnProducts && product.addOnProducts.length > 0) {
        ...
        for(var i=(productAddonsSize-1); i>=0; i--) {
          ...
          if(!product.addOnProducts[i].addOnOptions ||
             ...
             !product.addOnProducts[i].addOnOptions[0].product ||
