                    ...
                    if (prodListIds.length > 0) {
                        ccRestClient.request(ccConstants.ENDPOINT_PRODUCTS_LIST_PRODUCTS, prodListIdsObj, function(productList) {
                                ...
                                ccRestClient.request(ccConstants.ENDPOINT_PRODUCTS_AVAILABILITY, parameters, function(stockData) {
                                        ...
                                        console.log('items', getWidget.cart().items())
                                        var item = getWidget.cart().items();
                                        prodListIds.map(function(id) {
                                            item.map(function(e) {
                                                if (id == e.productId) {

                                                    e.externalData()['transNumber'] = widget.transNumber;
                                                    e.externalData()['type'] = widget.type;


                                                }
                                                // e.externalData = [{
                                                //                     "name": "vw_external_data",
                                                //                     "values": {
                                                //                         "vw_externalId": "test",
                                                //                         "vw_type": "contactlens"
                                                //                     }
                                        ...
                                        // item.map(function(e) {
                                        //     e.shopperInput = {}
                                        //     e.shopperInput['transNumber'] = widget.transNumber;
                                        //     e.shopperInput['type'] = widget.type;
                                        //     console.log('e', e)
                                        // });
                                        // widget.cart().updateCartItems();
                                        // console.log('updated items', getWidget.cart().items())
